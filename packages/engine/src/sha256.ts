// 纯 TypeScript 实现的 SHA-256 与 HMAC-SHA256（FIPS 180-4 / RFC 2104）。
// 不依赖 node:crypto，保证 engine 包在任何 JS 运行时（含未来浏览器端回放校验）行为一致。
// 正确性由 test/rng.test.ts 中的 NIST / RFC 4231 已知向量保证。
...
export function sha256Bytes(input: string | Uint8Array): Uint8Array {
  ...
  // 填充：1 bit + 若干 0 + 64 bit 长度
  const padded = new Uint8Array((((msg.length + 8) >> 6) + 1) << 6);
  padded.set(msg);
  padded[msg.length] = 0x80;
  const dv = new DataView(padded.buffer);
  // JS 位运算不支持 64 位；长度 < 2^53，高 32 位用除法取得
